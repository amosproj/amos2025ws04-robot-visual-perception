...
const getVideoStateBadgeClass = (state: string) => {
  if (state === 'connected') {
    return 'bg-gradient-to-br from-theme-success to-theme-success-secondary text-white text-base px-3 py-1 rounded';
  }
  if (state === 'connecting') {
    return 'bg-gradient-to-br from-theme-warning to-theme-warning-secondary text-white text-base px-3 py-1 rounded animate-pulse';
  }
  if (state === 'error') {
    return 'bg-gradient-to-br from-theme-error to-theme-error-secondary text-white text-base px-3 py-1 rounded';
  }
  return 'bg-theme-bg-tertiary text-theme-text-muted text-base px-3 py-1 rounded border border-theme-border';
};
    ...
    <div className="space-y-4 text-xl">
      ...
      {latencyMs !== undefined && latencyMs > 0 && (
        <div className="flex items-center justify-between">
          ...
          <span className="text-theme-accent text-lg font-mono">
            {latencyMs}ms
          </span>
        </div>
      )}
      ...
      <div className="flex items-center justify-between">
        ...
        <span className="text-theme-text-primary text-lg font-mono">
          {overlayFps} FPS
        </span>
      </div>
      ...
      <div className="flex items-center justify-between">
        ...
        <span
          className={`text-lg px-3 py-1 rounded ${objectCount > 0 ? 'bg-theme-accent text-white' : 'bg-theme-bg-tertiary text-theme-text-muted border border-theme-border'}`}
        >
          {objectCount}
        </span>
      </div>
