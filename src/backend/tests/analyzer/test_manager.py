...
async def test_process_detection_skips_non_detection_frames(
    manager: AnalyzerWebSocketManager,
    frame_id: int,
    sample_rate: int,
    should_detect: bool,
) -> None:
    """Test that _process_detection skips frames based on sample_rate."""
    ...
    detections, distances = await manager._process_detection(
        frame, state, detector, estimator
    )
...
@pytest.mark.asyncio
async def test_process_detection_returns_empty_when_no_active_connections(
    manager: AnalyzerWebSocketManager,
    processing_state: ProcessingState,
) -> None:
    """Test that _process_detection returns empty when no active connections."""
    ...
    detections, distances = await manager._process_detection(
        frame, processing_state, detector, estimator
    )
...
@pytest.mark.asyncio
async def test_process_detection_combines_detected_and_interpolated(
    manager: AnalyzerWebSocketManager,
    processing_state: ProcessingState,
) -> None:
    """Test that _process_detection combines detected and interpolated detections."""
    manager.active_connections.add(MagicMock())
    ...
    detections, distances = await manager._process_detection(
        MagicMock(), processing_state, detector, estimator
    )

    # should have both detected and interpolated
    assert len(detections) == 2
    assert len(distances) == 2
    assert detections[0].cls_id == 0  # detected
    assert detections[1].cls_id == 1  # interpolated
    assert distances == [2.5, 3.0]


@pytest.mark.asyncio
async def test_process_detection_excludes_updated_tracks_from_interpolation(
    manager: AnalyzerWebSocketManager,
    processing_state: ProcessingState,
) -> None:
    ...
    await manager._process_detection(MagicMock(), processing_state, detector, estimator)
